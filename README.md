...
# 使用方法，启动start_ui，在打开的ui界面获取对应的cookie，然后才能正常使用抖音下载。 其他的都没有测试。
...
1. 在终端中运行 `python main.py --web-ui` 命令启动 Web UI 模式
2. 启动后，访问 `http://127.0.0.1:5555/web` 可以使用 Web UI 界面

### Web UI 功能说明

- **首页**：显示项目简介和主要功能入口
- **链接下载**：输入抖音或 TikTok 链接，直接下载对应作品
- **账号管理**：添加和管理抖音/TikTok 账号，批量下载账号作品
- **下载设置**：自定义下载设置，包括保存路径、下载质量等
- **历史记录**：查看历史下载记录
